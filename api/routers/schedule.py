...
@router.get(
    "/schedules/{id}", response_model=ScheduleModel, response_model_by_alias=False
)
...
@router.get(
    "/schedules", response_model=ScheduleCollection, response_model_by_alias=False
)
async def list_schedules(
    ...
    if group is not None:
        ...
        if len(sc.streamers) > 0:
            codes.extend([s.code for s in sc.streamers if s.code is not None])
