...
class UserRepository:
    @staticmethod
    async def get_by_id(db: AgnosticDatabase, id: str) -> UserModel:
    ...
    @staticmethod
    async def get_by_username(db: AgnosticDatabase, username: str) -> UserModel | None:
        user_collection = db.get_collection("users")
        # ウォルラス演算子を利用して user が None でない場合に user を返す
    ...
    @staticmethod
    async def create(db: AgnosticDatabase, user: UserModel) -> UserModel:
        user_collection = db.get_collection("users")
        result = await user_collection.insert_one(
            user.model_dump(by_alias=True, exclude={"id"})
        )
        return await UserRepository.get_by_id(db, str(result.inserted_id))

    @staticmethod
    async def update(db: AgnosticDatabase, id: str, user: UserModel) -> UserModel:
        user_collection = db.get_collection("users")
