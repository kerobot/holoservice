...
class StreamerRepository:
    @staticmethod
    async def get_by_id(db: AgnosticDatabase, id: str) -> StreamerModel:
        streamer_collection = db.get_collection("streamers")
        # ウォルラス演算子を利用して streamer が None でない場合に streamer を返す
        if (
            streamer := await streamer_collection.find_one({"_id": ObjectId(id)})
        ) is not None:
            return StreamerModel(**streamer)
        raise NotFoundException(identifier=id)

    @staticmethod
    async def get_by_streamername(
        db: AgnosticDatabase, streamername: str
    ) -> StreamerModel:
        streamer_collection = db.get_collection("streamers")
        # ウォルラス演算子を利用して streamer が None でない場合に streamer を返す
        if (
            streamer := await streamer_collection.find_one({"name": streamername})
        ) is not None:
            return StreamerModel(**streamer)
    ...
    @staticmethod
    async def list(
        db: AgnosticDatabase, group: str | None = None
    ) -> StreamerCollection:
        streamer_collection = db.get_collection("streamers")
        filter_dict = {}
        ...
        return StreamerCollection(
            streamers=await streamer_collection.find(filter_dict)
            .sort("code", ASCENDING)
            .to_list(1000)
        )

    @staticmethod
    async def create(db: AgnosticDatabase, streamer: StreamerModel) -> StreamerModel:
        streamer_collection = db.get_collection("streamers")
        result = await streamer_collection.insert_one(
            streamer.model_dump(by_alias=True, exclude={"id"})
        )
        return await StreamerRepository.get_by_id(db, str(result.inserted_id))

    @staticmethod
    async def update(
        db: AgnosticDatabase, id: str, streamer: StreamerModel
    ) -> StreamerModel:
        streamer_collection = db.get_collection("streamers")
        ...
        if result is not None:
            return StreamerModel(**result)
        else:
            raise NotFoundException(identifier=id)

    @staticmethod
    async def delete(db: AgnosticDatabase, id: str):
        streamer_collection = db.get_collection("streamers")
